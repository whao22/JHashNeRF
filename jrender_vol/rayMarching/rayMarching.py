...
def render_rays(ray_batch,
    ...
    pts = rays_o.unsqueeze(-2) + rays_d.unsqueeze(-2) * \
        z_vals.unsqueeze(-1)  # [N_rays, N_samples, 3]

    raw = network_query_fn(pts, viewdirs, network_fn)
    rgb_map, disp_map, acc_map, weights, depth_map, sparsity_loss = integrator(
    ...
    if N_importance > 0:
        ...
        rgb_map, disp_map, acc_map, weights, depth_map, sparsity_loss = integrator(
            raw, z_vals, rays_d, raw_noise_std, white_bkgd)
        rgb_map_1, disp_map_1, acc_map_1, sparsity_loss_1 = rgb_map, disp_map, acc_map, sparsity_loss

        N_importance1 = 2*N_importance
        if N_importance1 > 0:
            # importance sampling
            z_vals_mid = .5 * (z_vals[..., 1:] + z_vals[..., :-1])
            z_samples = sample_pdf(
                z_vals_mid, weights[..., 1:-1], N_importance1, det=(perturb == 0.))
            z_samples = z_samples.detach()
            ...
            run_fn = network_fn if network_fine1 is None else network_fine1
            raw = network_query_fn(pts, viewdirs, run_fn)
            rgb_map, disp_map, acc_map, weights, depth_map, sparsity_loss = integrator(
                raw, z_vals, rays_d, raw_noise_std, white_bkgd)
    ...
    if N_importance > 0:
        ...
        ret['rgb1'] = rgb_map_1
        ret['disp1'] = disp_map_1
        ret['acc1'] = acc_map_1
        ret['sparsity_loss1'] = sparsity_loss_1
        ret['z_std'] = jt.Var(np.std(z_samples.numpy(), axis=-1))  # [N_rays]
